...
for index, file_name in enumerate(json_files, 1):
    ...
    for idx, c in enumerate(chunks):
        ...
        # 청킹 단계의 chunk_id를 Chroma/MySQL 공통 식별자로 사용
        c["generated_id"] = c.get("chunk_id") or f"emb_{doc_uuid}_p{page_num}_{idx}"
    ...
    for i in range(0, len(new_chunks), BATCH_SIZE):
        ...
        for c in batch:
            meta_obj = c.get("metadata", {})
            meta = {
                "chunk_id": str(c.get("chunk_id") or c.get("generated_id")),
                "document_id": str(meta_obj.get("document_uuid")),
        ...
        if db_store and db_store.mysql_enabled():
            try:
                ...
                print(f"MySQL 임베딩 상태 업데이트 실패: {db_error}")
