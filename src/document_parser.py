...
def count_pdf_pages(pdf_path: str):
    with open(pdf_path, "rb") as pdf_file:
        return sum(1 for _ in PDFPage.get_pages(pdf_file))


def infer_page_count(file_path: str, text: str):
    file_path = Path(file_path)

    if file_path.suffix.lower() == ".pdf":
        return count_pdf_pages(str(file_path))

    page_numbers = [
        int(match.group(1))
        for match in re.finditer(r"\bPage\s+(\d+)\b", text or "", flags=re.IGNORECASE)
    ]
...
def build_rag_document(
    ...
    page_count = infer_page_count(str(source_path), full_text)
...
def build_parser():
    ...
    parser.add_argument("file_path", help="파싱할 개인 PDF/HTML/XML/TXT 파일 경로")
